...
export default function Ensemble() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setProbability(
        response.data.Average_Probabilities
          ? Math.max(...response.data.Average_Probabilities)
          : null
      )
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-200 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full transition-all duration-300 ease-in-out transform hover:scale-105">
        <h1 className="text-3xl font-bold text-center mb-6 text-purple-700">
          Ensemble Model: Fire & Smoke Detector
        </h1>
        <div className="space-y-6">
          <div className="relative">
            ...
            <label
              htmlFor="file-upload"
              className="flex items-center justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none cursor-pointer hover:border-purple-600 focus:outline-none"
            >
              <span className="flex items-center space-x-2">
                <Upload className="w-6 h-6 text-purple-600" />
                <span className="font-medium text-gray-600">
                  Drop files to Attach, or <span className="text-purple-600 underline">browse</span>
                </span>
              </span>
            </label>
          </div>
          ...
          <button
            className={`w-full py-2 px-4 bg-purple-600 text-white rounded-md font-semibold shadow-md transition-all duration-300 ease-in-out transform hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50 ${
          ...
          <div className={`mt-6 p-4 rounded-md ${
            ...
            {probability !== null && (
              <div className="flex items-center space-x-2">
                ...
                <p className="text-sm">
                  Probability of {prediction}: {(probability * 100).toFixed(2)}%
                </p>
              </div>
            )}
          </div>
